...
class Product
{
    ...
    c.id AS category_id, c.name AS category_name, pd.id AS product_details_id, pd.designation AS designatory,
    ...
    {
        $sql = $this->commonSql . ' WHERE expired_status = 0 AND stock_status = 0 OR stock_status = 2 OR stock_status = 3 AND designatory = 0' . $this->groupBySql;
        $result = $this->conn->query($sql);
    ...
    public function updateOverstock()
    {
        ...
        $sql = "UPDATE products 
                LEFT JOIN (
                    SELECT product_id, SUM(quantity) AS total_quantity, designation
                    FROM product_details
                    GROUP BY product_id
                ) AS pd ON pd.product_id = products.id
                SET stock_status = 3
                WHERE pd.total_quantity > max_stock AND pd.designation = 0;";
    ...
    public function updateUnderstock()
    {
        ...
        SET stock_status = 2 
        WHERE pd.total_quantity < min_stock AND pd.designation = 0;";
    ...
    public function updateOutofStock()
    {
        ...
        SET stock_status = 0 
        WHERE pd.total_quantity = 0 AND pd.designation = 0;";
    ...
    // public function updateDesignatedStock()
    // {
    //     // $quantity = "SELECT quantity FROM product_details INNER JOIN products p ON p.id = quantity";

    //     $sql = "UPDATE products 
    //     LEFT JOIN (
    //                 SELECT product_id, designation
    //                 FROM product_details
    //                 GROUP BY product_id
    //             ) AS pd ON pd.product_id = products.id
    //     SET stock_status = 4 
    //     WHERE pd.designation = 1 OR pd.designation = 2 OR pd.designation = 3;";
